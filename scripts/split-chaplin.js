// Script: separa los cortos de Chaplin (Essanay y Mutual) en entradas individuales
// Uso: node scripts/split-chaplin.js
...
async function main() {
  ...
  // Find and remove the two collection entries
  const essanayId = library.find(m => m.folderName?.includes('Essanay'))?.id;
  const mutualId  = library.find(m => m.folderName?.includes('Mutual'))?.id;
  const cleaned   = library.filter(m => m.id !== essanayId && m.id !== mutualId);
  console.log(`Removidas las 2 entradas de colección. Librería: ${library.length} → ${cleaned.length}`);
  ...
  // Insert new entries where the collections were (at the same position as essanay)
  const insertIdx = library.findIndex(m => m.id === essanayId);
  const final = [...cleaned];
  final.splice(insertIdx >= 0 ? insertIdx : final.length, 0, ...newEntries);
